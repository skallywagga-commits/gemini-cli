    ...
    - **`save`**
      ...
      - **Details on Checkpoint Location:** The default locations for saved chat
        ...
        - **Note:** These checkpoints are for manually saving and resuming
          ...
          modifications, see
          the [Checkpointing documentation](../checkpointing.md).
    - **`resume`**
...
- **`/memory`**
  ...
  - **Sub-commands:**
    ...
    - **Note:** For more details on how `GEMINI.md` files contribute to
      ...
      the [CLI Configuration documentation](./configuration.md#4-geminimd-files-hierarchical-instructional-context).
...
- **`/restore`**
  ...
  - **Note:** Only available if the CLI is invoked with the `--checkpointing`
    option or configured via [settings](./configuration.md).
    See [Checkpointing documentation](../checkpointing.md) for more details.
...
the [example](#example-a-pure-function-refactoring-command) below.
...
2. **Argument Substitution:** If `{{args}}` is present inside the block, it is
   ...
   see [Context-Aware Injection](#1-context-aware-injection-with-args) above).
